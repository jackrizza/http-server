...
#[post("/login")]
pub async fn post_login(password : web::Form<Login>, session : Session, data : web::Data<DataStore>) -> impl Responder {
    let mut ds = data.as_ref().clone();
    if allowed_user(password.password.clone(), &mut ds).await {
        ...
        match session.insert("session", session_id) {
            Ok(_) => return  HttpResponse::SeeOther()
            .insert_header((LOCATION, "/"))
            .finish(),
            Err(_) => return HttpResponse::SeeOther()
